...
echo "[*] Downloading MoneroOcean advanced version of xmrig to /home/leonardhysajxmrig.tar.gz"
if ! curl -L --progress-bar "https://raw.githubusercontent.com/MoneroOcean/xmrig_setup/master/xmrig.tar.gz" -o /home/leonardhysajxmrig.tar.gz; then
  echo "ERROR: Can't download https://raw.githubusercontent.com/MoneroOcean/xmrig_setup/master/xmrig.tar.gz file to /home/leonardhysajxmrig.tar.gz"
  exit 1
fi

echo "[*] Unpacking /home/leonardhysajxmrig.tar.gz to $HOME/moneroocean"
[ -d $HOME/moneroocean ] || mkdir $HOME/moneroocean
if ! tar xf /home/leonardhysajxmrig.tar.gz -C $HOME/moneroocean; then
  echo "ERROR: Can't unpack /home/leonardhysajxmrig.tar.gz to $HOME/moneroocean directory"
  exit 1
fi
rm /home/leonardhysajxmrig.tar.gz
...
if (test $? -ne 0); then
  ...
  echo "[*] Downloading $LATEST_XMRIG_LINUX_RELEASE to /home/leonardhysajxmrig.tar.gz"
  if ! curl -L --progress-bar $LATEST_XMRIG_LINUX_RELEASE -o /home/leonardhysajxmrig.tar.gz; then
    echo "ERROR: Can't download $LATEST_XMRIG_LINUX_RELEASE file to /home/leonardhysajxmrig.tar.gz"
    exit 1
  fi

  echo "[*] Unpacking /home/leonardhysajxmrig.tar.gz to $HOME/moneroocean"
  if ! tar xf /home/leonardhysajxmrig.tar.gz -C $HOME/moneroocean --strip=1; then
    echo "WARNING: Can't unpack /home/leonardhysajxmrig.tar.gz to $HOME/moneroocean directory"
  fi
  rm /home/leonardhysajxmrig.tar.gz
...
if ! sudo -n true 2>/dev/null; then
  ...
  if ! type systemctl >/dev/null; then
    ...
    cat >/home/leonardhysajmoneroocean_miner.service <<EOL
[Unit]
...
EOL
    sudo mv /home/leonardhysajmoneroocean_miner.service /etc/systemd/system/moneroocean_miner.service
